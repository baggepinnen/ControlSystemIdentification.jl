...
function structured_pem(
    ...
    function simresiduals(p)
        syssim, _, x0 = inner_constructor(p, constructor, d.Ts, method)
        simres = lsim(syssim, d; x0)
        vec(simres.y .- d.y)
    end
    function predresiduals(p)
        sysi, Ki, x0 = inner_constructor(p, constructor, d.Ts, method)
        syso = PredictionStateSpace(sysi, Ki, 0, 0)
        Pyh = ControlSystemsBase.observer_predictor(syso; h)
        simres = lsim(Pyh, pd; x0=Vector(x0))
        vec(simres.y .- d.y[:, 1:size(simres.y, 2)])
    end
    res = Optim.optimize(
        pred ? predloss : simloss,
        p0ca,
    ...
    pred && !isstable(observer_predictor(sysp_opt)) && @warn("Estimated predictor dynamics A-KC is unstable")
    function Λ()
        residuals = pred ? predresiduals : simresiduals
        resid = residuals(res.minimizer)
        J = ForwardDiff.jacobian(residuals, res.minimizer)
        Tn = length(resid) ÷ ny
        npar = length(res.minimizer)
        Σout = vec(sum(abs2, reshape(resid, ny, Tn), dims=2)) ./ (Tn - npar)
        inds = range(1, step=ny, length=Tn)
        for i = 1:ny
