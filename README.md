...
```
We can also estimate a transfer function using spectral techniques, the main entrypoint to this is the function `tfest`:
```julia
G = tfest(h,yn,u)
bodeplot(sys, exp10.(range(-3, stop=log10(pi), length=200)), layout=(2,1), plotphase=false, subplot=1)
